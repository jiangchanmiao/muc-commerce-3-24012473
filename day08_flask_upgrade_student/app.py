...
@login_required
def metrics_api():
    # TOD 8-1：返回四张指标卡的JSON数据，并保留label、value、note字段。
    raw = load_metric_api_data(BASE_DIR)
    metrics = [
...
def categories_api():
    category = request.args.get("category", "全部")
    # TOD 8-2：将category查询参数传给数据服务，返回筛选后的表格记录。
    rows = load_category_api_data(BASE_DIR, category=category)
...
def bad_request(_error):
    # TOD 8-3：统一返回JSON错误结构，至少包含ok和error字段。
