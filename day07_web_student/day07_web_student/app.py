...
from services.data_service import filter_category_export, load_dashboard_data
...
def download():
    category = request.args.get("category", "全部")
    export_df = filter_category_export(BASE_DIR, category)
    buffer = BytesIO()
    ...
    return send_file(
        ...
        mimetype="text/csv",
        as_attachment=True,
        download_name=f"category_analysis_{safe_name}.csv",
    )
